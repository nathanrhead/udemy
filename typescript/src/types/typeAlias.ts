...
let mystery: unknown = 'Ian wuz ere.';
const numChars = (mystery as string).length; // It won't change's mystery's type permanently.
const input = document.getElementById('btn') as HTMLInputElement; // Without casting it to an HTMLInputElement type, it is assigned the generic HTMLElement type, which doesn't have access to the "value" key.
